...
import com.test.gang.proxy.HttpProxyCacheServer
...
public class MediaPlayerManager {
    ...
    // 视频加载代理
    @Volatile
    private lateinit var mProxyCacheServer: HttpProxyCacheServer
    ...
    fun buildDataSource(context: Context, uri: String): MediaSource {
        // 构建一个默认的Http数据资源处理工厂
        val mHttpDataSourceFactory = DemoUtil.getHttpDataSourceFactory(context)
        ...
        return ProgressiveMediaSource.Factory(mDataSourceFactory).createMediaSource(MediaItem.fromUri(getProxyUrl(uri)))
    }
     ...
     * 获取代理地址
     */
    fun getProxyUrl(url: Uri): Uri = Uri.parse(mProxyCacheServer.getProxyUrl(url.toString()))
    /**
     * 是否缓存
     * @return true:已经缓存
     */
