...
public class IntentUtil {
    ...
    public static List<MediaItem> createMediaItemsFromIntent(Context context,Intent intent) {
        List<MediaItem> mediaItems = new ArrayList<>();
        if (ACTION_VIEW_LIST.equals(intent.getAction())) {
            int index = 0;
            while (intent.hasExtra(URI_EXTRA + "_" + index)) {
                Uri uri = Uri.parse(intent.getStringExtra(URI_EXTRA + "_" + index));
                mediaItems.add(createMediaItemFromIntent(context, uri, intent, /* extrasKeySuffix= */ "_" + index));
                index++;
            }
        } else {
            Uri uri = intent.getData();
            mediaItems.add(createMediaItemFromIntent(context, uri, intent, /* extrasKeySuffix= */ ""));
        }
        return mediaItems;
    }
    ...
    private static MediaItem createMediaItemFromIntent(Context context,
        ...
        MediaPlayerManager mediaProxyManager = MediaPlayerManager.Companion.getDefault();
        mediaProxyManager.init(context, "test_player");
        Uri proxyUri = mediaProxyManager.getProxyUrl(uri);
